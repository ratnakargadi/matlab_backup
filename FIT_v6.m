...
T = @(X)reshape(repmat(prof(1).temp(50:200),[1 4]) + repmat((prof(4).temp(50:200) - prof(1).temp(50:200))/2,[1 4]) .* (tanh(ETA./Ty - X(1) * Z) + 1),[151*4 1]);
xon = 0;
options = optimset('MaxFunEvals',80000,'MaxIter',80000,'Display','iter','TolFun',10^-7);
[xp,resnorm,residual,exitflag,output] = lsqnonlin(T,xon,[],[],options);

T = @(ETA,Z,X)(repmat(prof(1).temp(50:200),[1 4]) + repmat((prof(4).temp(50:200) - prof(1).temp(50:200))/2,[1 4]) .* (tanh(ETA./Ty - X(1) * Z) + 1));
