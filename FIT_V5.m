...
F = fittype('1/(pi * gamma) * (1 + ((eta - eta_o)/gamma)^2)^C','Coefficients',{'C','gamma','eta_o'},'independent','eta');
...
for i=depth_start_ind:depth_end_ind
    ...
    count = count + 1;
    close all;
    plot([prof.dist_along]',[prof(1).salt(i) prof(2).salt(i) prof(3).salt(i) prof(4).salt(i)]');
    hold on;
    plot([prof.dist_along],D([prof.dist_along]),'g');
end
[Dum,iter] = med_filt([CTD_depth(depth_start_ind:depth_end_ind),C'],13,300);
C = Dum(:,2);
fit_CC = fit(-CTD_depth(depth_start_ind:depth_end_ind),C,'smoothingspline');
[Dum,iter] = med_filt([CTD_depth(depth_start_ind:depth_end_ind),A'],13,300);
A = Dum(:,2);
fit_AA = fit(-CTD_depth(depth_start_ind:depth_end_ind),A,'smoothingspline');
eta_o(abs(eta_o)>200) = mean(eta_o(abs(eta_o)<=200));
Nand = fit(-CTD_depth(depth_start_ind:depth_end_ind),eta_o','smoothingspline');
Final_func_s = @(eta,z)1./(pi * reshape(fit_A(z),size(z))) .* ....
    (1 + ((eta -reshape( Nand(z),size(z)))./reshape(fit_A(z),size(z))).^2).^(reshape(fit_C(z),size(z)));
