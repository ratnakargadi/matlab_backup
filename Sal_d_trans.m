...
plot_dir_s = [pwd filesep 'salt'];
if(~exist(plot_dir_s))
    mkdir(plot_dir_s);
end

plot_dir_t = [pwd filesep 'temp'];
if(~exist(plot_dir_t))
    mkdir(plot_dir_t);
end

plot_dir_v = [pwd filesep 'vel'];
if(~exist(plot_dir_v))
    mkdir(plot_dir_v);
end
...
Final_func_s = @(eta,z,sigma_b,sigma_t,sigma_bot)reshape(func1(z),size(z)).*exp(-((eta).^2./(2*heaviside_fit(z,sigma_b,sigma_t,sigma_bot).^2)).^9);
...
count = 1;
for i=depth_start_ind:depth_end_ind
    D = fit([prof.dist_along]',[prof(1).temp(i) prof(2).temp(i) prof(3).temp(i) prof(4).temp(i)]',G,'problem',{prof(1).temp(i),prof(4).temp(i)},options);
    gamma(count) = D.gamma;
    T1(count) = prof(1).temp(i);
    T2(count) = prof(4).temp(i);
    count = count + 1;
end
[func2,gof] = fit( -CTD_depth(depth_start_ind:depth_end_ind), gamma',  'smoothingspline' );
Final_func_t = @(eta,z,T1,T2)T1 +  (T2 - T1) *(tanh(eta./reshape(func2(z),size(z))) + 1)/2;
clear eta;
...
plot(Tp,-CTD_depth(depth_start_ind:depth_end_ind),'g');
RMSE = sqrt(mean((Tp' - prof(1).temp(depth_start_ind:depth_end_ind)).^2,'omitNaN'));
xlabel('Temp(C)');
ylabel('Depth(m)');
ylim([-depth_end -depth_start]);
legend('Actual','Fit');
...
plot(Tp,-CTD_depth(depth_start_ind:depth_end_ind),'g');
RMSE = sqrt(mean((Tp' - prof(2).temp(depth_start_ind:depth_end_ind)).^2,'omitNaN'));
xlabel('Temp(C)');
ylabel('Depth(m)');
ylim([-depth_end -depth_start]);
legend('Actual','Fit');
...
plot(Tp,-CTD_depth(depth_start_ind:depth_end_ind),'g');
RMSE = sqrt(mean((Tp' - prof(3).temp(depth_start_ind:depth_end_ind)).^2,'omitNaN'));
xlabel('Temp(C)');
ylabel('Depth(m)');
ylim([-depth_end -depth_start]);
legend('Actual','Fit');
...
plot(Tp,-CTD_depth(depth_start_ind:depth_end_ind),'g');
RMSE = sqrt(mean((Tp' - prof(4).temp(depth_start_ind:depth_end_ind)).^2,'omitNaN'));xlabel('Salt(psu)');
ylabel('Depth(m)');
ylim([-depth_end -depth_start]);
legend('Actual','Fit');
title(sprintf('Profile taken at %4.2f E and %4.2f N(RMSE=%4.2f psu)',prof(4).lon,prof(4).lat,RMSE));
