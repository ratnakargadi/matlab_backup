%% This code writes the mean climatology fields into netcdf files and writes
% them in the same format as what nc2mods expects.
clear all;
clc;
close all;

Tfile = 'Climatology_meanT.nc';
Sfile = 'Climatology_meanS.nc';
file = 'Climatology.nc';
if(exist(file))
    delete(file);
end
...
temp = ncread(Tfile,'t_an');
depth = ncread(Tfile,'depth');
lat = ncread(Tfile,'lat');
lon = ncread(Tfile,'lon');
salt = ncread(Sfile,'s_an');
...
argo_inname = file;
argo_outname = fullfile(mods_file_dir,'Climatology');
lonlim = extrem(tlon(:));
latlim = extrem(tlat(:));
htitle = 'Climatology from WOAA database';
argo_nc2mods;
fprintf('%s ---- Done!!\n\n',argo_inname);